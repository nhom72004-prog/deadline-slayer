...
@media(prefers-color-scheme:dark){
    .friend-card{background:#1e1f22;border-color:#2b2d31;}
}
...
def init_spreadsheet_structure(ss):
    ...
    for name, cols in {WS_TASKS:TASK_COLS,WS_USERS:USER_COLS,WS_GROUPS:GROUP_COLS,
                       WS_PROOFS:PROOF_COLS,WS_CHAT:CHAT_COLS,WS_DM:DM_COLS}.items():
...
def fetch_all_data():
    ...
    empty = {"tasks":pd.DataFrame(columns=TASK_COLS),"users":pd.DataFrame(columns=USER_COLS),
...
def calc_status(row):
    if str(row.get("Trạng_Thái",""))=="Đã xong" or clean_progress(row.get("Tiến_Độ_%"))==100: return "done"
    ...
    with col2:
        ...
        with t1:
            ...
            if st.button("🚀 Đăng Nhập",use_container_width=True,type="primary"):
                ...
                if fu.empty: st.error("👻 Chưa có ai! Đăng ký tài khoản đầu tiên đi!")
                else:
                    m=fu[(fu["User_ID"]==log_id)&(fu["Password"]==log_pass)]
                    if not m.empty:
                        st.session_state.update({"logged_in":True,"current_user":m.iloc[0].to_dict()})
                        st.success(msg_login_success(m.iloc[0]["Tên"])); st.rerun()
                    else: st.error(msg_login_fail())
        with t2:
            ...
            if st.button("✨ Tạo Tài Khoản",use_container_width=True):
                if not rn or not re or not rp: st.error("🙏 Điền đủ thông tin nhé!")
                else:
                    fetch_all_data.clear()
                    fu=fetch_all_data()["users"]
                    if not fu.empty and re in fu["Email"].values: st.error("📧 Email đã tồn tại!")
                    else:
                        ...
                        append_row_data(WS_USERS,[new_id,rp,rn,re,"",rw,NOW().strftime("%Y-%m-%d %H:%M:%S")])
                        fetch_all_data.clear()
...
def main_app(data):
    ...
    with st.sidebar:
        ...
        if st.button("🚪 Đăng xuất",use_container_width=True):
            st.session_state.update({"logged_in":False,"current_user":None}); st.rerun()
        st.markdown("---")
        ...
        if st.button("💾 Lưu Webhook",use_container_width=True):
            ...
            fetch_all_data.clear(); st.toast(msg_webhook_saved())
        if is_leader:
            st.markdown("---")
            ...
            if st.button("🚀 Bắn Lên Discord",use_container_width=True,type="primary"):
...
def render_dashboard(tasks_df,groups_df,users_df,my_id,is_leader):
    ...
    if vt.empty:
        st.info("🎉 Chưa có nhiệm vụ nào — tận hưởng khoảnh khắc này đi!"); return
    ...
    for _,row in vt.iterrows():
        ...
        st.markdown(f"""<div style="border:1px solid {bc};border-left:5px solid {bc};background:#111112;
            border-radius:10px;padding:12px 16px;margin-bottom:8px;">
            <div style="display:flex;justify-content:space-between;">
                <b>📌 {row['Tên_Công_Việc']}</b>
                <span style="color:{bc};font-weight:bold;">{slabels.get(row['_st'],'')}</span>
            </div>
            <div style="font-size:13px;color:#aaa;margin-top:5px;">👤 <b>{an}</b> | ⏰ {row['Deadline']}</div>
            <div style="font-size:14px;color:{bc};font-weight:500;margin-top:3px;">{row['_rem']} | {row['Tiến_Độ_%']}%</div>
            ...
            with st.expander("🛠 Cập nhật & Nộp Minh Chứng"):
                ...
                with c1:
                    ...
                    if st.button("💾 Lưu",key=f"btn_{row['ID']}"):
                        ...
                        if np_==100: update_cell_by_id(WS_TASKS,"ID",row["ID"],"Trạng_Thái","Đã xong",TASK_COLS)
                        st.success(msg_progress_saved(np_)); st.rerun()
                with c2:
...
def render_network_and_tasks(users_df,groups_df,tasks_df,my_id,my_friends_list):
    ...
    with sub1:
        ...
        with c1:
            ...
            grp_name=st.text_input("Tên nhóm:")
            grp_wh=st.text_input("🤖 Discord Webhook (tuỳ chọn):",placeholder="https://discord.com/api/webhooks/...").strip()
            fo={f:f"{get_user_name(f,users_df)} ({f})" for f in my_friends_list}
            sel_f=st.multiselect("Chọn đồng đội:",list(fo.keys()),format_func=lambda x:fo[x])
            if st.button("🚀 Thành lập nhóm!",type="primary"):
                if not grp_name: st.error("✏️ Đặt tên nhóm đi!")
                    ...
                    append_row_data(WS_GROUPS,[new_gid,grp_name,my_id,",".join([my_id]+sel_f),grp_wh,NOW().strftime("%Y-%m-%d")])
                    if grp_wh: push_to_discord(discord_group_created(grp_name),grp_wh)
                    st.success(msg_group_created(grp_name)); st.rerun()
        ...
        if mld.empty: st.info("Bạn chưa làm trưởng nhóm nào!")
        else:
            ...
            fo2={f:f"{get_user_name(f,users_df)} ({f})" for f in my_friends_list}
            with st.expander(f"🛠 Chỉnh sửa: {gd['Tên_Nhóm']}",expanded=True):
                ...
                nm=st.multiselect("Thành viên:",list(fo2.keys()),default=vcm,format_func=lambda x:fo2[x],key=f"m_{egid}")
                cs,cd=st.columns(2)
                with cs:
                    if st.button("💾 Lưu",type="primary",use_container_width=True):
                        update_cell_by_id(WS_GROUPS,"Group_ID",egid,"Tên_Nhóm",ngn,GROUP_COLS)
                        update_cell_by_id(WS_GROUPS,"Group_ID",egid,"Thành_Viên_IDs",",".join([my_id]+nm),GROUP_COLS)
                        update_cell_by_id(WS_GROUPS,"Group_ID",egid,"Discord_Webhook",nwh,GROUP_COLS)
                        st.success(msg_group_updated()); st.rerun()
                with cd:
                    if st.button("💥 Giải tán",use_container_width=True):
                        delete_row_by_id(WS_GROUPS,"Group_ID",egid,GROUP_COLS)
                        st.success(msg_group_deleted()); st.rerun()

    with sub2:
        st.subheader("📋 Giao Việc")
        ao={my_id:f"🙋 Tự mình ({get_user_name(my_id,users_df)})"}
        for f in my_friends_list: ao[f]=f"👤 {get_user_name(f,users_df)}"
        with st.form("assign_form"):
            tn=st.text_input("Tên Nhiệm Vụ *")
            sj=st.text_input("Môn học")
            aid=st.selectbox("Giao cho:",list(ao.keys()),format_func=lambda x:ao[x])
            cc1,cc2=st.columns(2)
            with cc1: dd=st.date_input("Ngày deadline",min_value=NOW().date())
            with cc2: dt=st.time_input("Giờ")
            pri=st.selectbox("Độ ưu tiên",["Cao","Trung bình","Thấp"])
            notes=st.text_area("Ghi chú")
            if st.form_submit_button("⚡ Phát Lệnh!"):
                if not tn: st.error("✏️ Đặt tên nhiệm vụ đi!")
                else:
                    nums=[int(i[1:]) for i in tasks_df["ID"].dropna().astype(str).tolist()
                    ...
                    append_row_data(WS_TASKS,[new_tid,tn,sj,aid,fdl,pri,"Chưa xong",0,"Bắt đầu",notes,5,"",NOW().strftime("%Y-%m-%d %H:%M:%S"),""])
                    ...
                    for _,g in groups_df[groups_df["Thành_Viên_IDs"].str.contains(aid,na=False)].iterrows():
                        ...
                        if wh and wh not in notified:
                            push_to_discord(discord_task_assigned(tn,an,fdl,pri),wh); notified.add(wh)
...
def render_chat(chat_df,dm_df,groups_df,users_df,my_id,my_friends_list):
    ...
    with sg:
        ...
        if mjg.empty:
            ...
            sgid=st.selectbox("Chọn nhóm:",list(go.keys()),format_func=lambda x:go[x],key="cgs")
            ...
            with st.container(height=400):
                ...
                if gc.empty:
                    ...
                    st.markdown(render_messages_html(gc.iterrows(),my_id,users_df,"group"),unsafe_allow_html=True)
            ...
            with cd:
                ...
                with st.form("grp_disc_form",clear_on_submit=True):
                    ...
                    if sd:
                        ...
                        elif not wh_grp: st.error("❌ Chưa có Webhook!")
                        else:
                            dm=discord_group_chat(sname,glabel,md.strip()) if md.strip() else f"📎 **{sname}** gửi file vào **{glabel}**!"
    ...
    with sdm:
        if not my_friends_list:
            ...
            fo={f:f"{get_user_name(f,users_df)} ({f})" for f in my_friends_list}
            sf=st.selectbox("Nhắn với ai?",list(fo.keys()),format_func=lambda x:fo[x],key="dfs")
            ...
            with dw:
                ...
                with st.form("dm_web_form",clear_on_submit=True):
                    dmw=st.text_area("Nội dung:",placeholder=f"Nhắn riêng cho {fn}...",height=80,key="dwm")
                    dfw=st.file_uploader("📎 File đính kèm",key="dwf")
                    also_disc_dm=st.checkbox("📡 Đồng thời gửi Discord của họ",value=bool(rwh),key="dwd_also")
                    sdw=st.form_submit_button("📩 Gửi lên Web",use_container_width=True)
                    if sdw:
                        if not dmw.strip() and not dfw: st.warning("💭 Nhắn gì đi~")
                        else:
            ...
            with dd2:
                st.markdown("#### 🎮 Gửi lên Discord")
                if rwh: st.markdown(f'<span class="disc-badge disc-on">🟢 {fn} sẽ nhận ping Discord</span>',unsafe_allow_html=True)
                ...
                with st.form("dm_disc_form",clear_on_submit=True):
                    ...
                    if sdd:
                        ...
                        elif not rwh: st.error(f"❌ {fn} chưa cài Webhook!")
                        else:
                            dm2=discord_dm(sname,dmd.strip()) if dmd.strip() else f"📎 **{sname}** gửi file cho bạn~"
                            ok=push_to_discord(dm2,rwh,dfd.getvalue(),dfd.name) if dfd else push_to_discord(dm2,rwh)
...
def render_friends_management(users_df, my_id, my_friends_list):
    ...
    with c1:
        ...
        if st.button("🤝 Kết bạn nào!",use_container_width=True):
            if sid==my_id: st.warning("🪞 Không thể kết bạn với chính mình 😂")
            elif sid in my_friends_list: st.info("👯 Đã là bạn bè rồi!")
            else:
                tu=users_df[users_df["User_ID"]==sid]
                if tu.empty: st.error("🔍 Không tìm thấy! Kiểm tra lại ID~")
                else:
        ...
        if not my_friends_list:
            ...
            for fid in my_friends_list:
                ...
                if st.button(f"❌ Xóa {fname}",key=f"rm_{fid}",use_container_width=True):
                    nf=[f for f in my_friends_list if f!=fid]
                    update_cell_by_id(WS_USERS,"User_ID",my_id,"Bạn_Bè",",".join(nf),USER_COLS)
                    st.success(msg_friend_removed(fname)); st.rerun()
...
def render_leaderboard(tasks_df,users_df):
    ...
    if tasks_df.empty: st.info("🎲 Chưa có task nào!"); return
    t=tasks_df.copy()
    ...
    g=t.groupby("Người_Phụ_Trách_ID").apply(lambda x:pd.Series({
        "Tổng Task":len(x),"Đã Xong":(x["_st"]=="done").sum(),
        "Tiến độ TB":f"{int(x['Tiến_Độ_%'].mean())}%"})).reset_index()
    g["🏅 Chiến Binh"]=g["Người_Phụ_Trách_ID"].apply(lambda x:get_user_name(x,users_df))
    g=g[["🏅 Chiến Binh","Tổng Task","Đã Xong","Tiến độ TB"]].sort_values("Đã Xong",ascending=False).reset_index(drop=True)
    g.index+=1
    medals={1:"🥇",2:"🥈",3:"🥉"}
...
def render_account_tab(users_df,my_id):
    ...
    if users_df.empty: st.info("👻 Hệ thống trống!"); return
    ...
    if others.empty: st.info("😎 Chỉ có mình bạn!"); return
    opts={r["User_ID"]:f"{r['Tên']} ({r['User_ID']}) — {r['Email']}" for _,r in others.iterrows()}
    ...
    if st.checkbox(f"✅ Xác nhận xóa `{did}`"):
        if st.button("💥 Xóa Tài Khoản",type="primary"):
            if delete_row_by_id(WS_USERS,"User_ID",did,USER_COLS):
                st.success(f"💨 Đã xóa `{did}`!"); fetch_all_data.clear(); st.rerun()
            else: st.error("😵 Xóa không được! Refresh thử~")
...
if __name__=="__main__":
    data=fetch_all_data()
    if not st.session_state["logged_in"]: show_auth_page(data)
    else: main_app(data)