...
# Mapping tên cột cũ → mới cho TẤT CẢ các sheet
COLUMN_ALIAS = {
    # Tasks - tên cũ có thể thiếu hậu tố _ID hoặc viết khác
    "Người_Phụ_Trác":       "Người_Phụ_Trách_ID",  # thiếu h + ID
    "Người_Phụ_Trách":      "Người_Phụ_Trách_ID",  # thiếu _ID
...
def migrate_sheet(ws, expected_cols):
    """
    Đọc header hiện tại của sheet, rename cột cũ → mới,
    append các cột còn thiếu vào cuối header row.
    Không đụng đến dữ liệu.
    """
    try:
        ...
        # Bước 2: thêm cột thiếu vào cuối
        missing = [c for c in expected_cols if c not in updated]
        if missing:
            updated += missing
        # Bước 3: chỉ ghi lại nếu có thay đổi
        if updated != current:
            # Ghi header mới (chỉ ghi đúng số cột hiện có + cột mới)
            cell_range = f"1:1"
            ws.update(cell_range, [updated])
    except Exception:
        pass  # silent — không làm crash app
...
def fetch_all_data():
    ...
    try:
        ...
        def get_df(name, cols):
            ...
            rows   = vals[1:]
            # Trim/pad each row to match header length
            n = len(header)
...
def main_app(data):
    ...
    # Refresh user từ sheet — guard stale session
    ...
    with sub1:
        ...
        with c1:
            ...
            grp_wh   = st.text_input("🤖 Discord Webhook (tuỳ chọn):",
                                     placeholder="https://discord.com/api/webhooks/...").strip()
                    ...
                    if st.form_submit_button("📩 Gửi lên Web", use_container_width=True):
                        if not mw.strip() and not fw:
                            ...
                            if also_disc and wh_grp:
                                dm = (discord_group_chat(sname, glabel, mw.strip()) if mw.strip()
                                      else f"📎 **{sname}** gửi file vào **{glabel}**!")
                                push_to_discord(dm, wh_grp, fw.getvalue(), fw.name) if fw \
                ...
                with st.form("grp_disc_form", clear_on_submit=True):
                    ...
                    if st.form_submit_button("🚀 Gửi lên Discord", use_container_width=True,
                        ...
                        if not md.strip() and not fd:
                            ...
                            dm = (discord_group_chat(sname, glabel, md.strip()) if md.strip()
                                  else f"📎 **{sname}** gửi file vào **{glabel}**!")
                            ok = push_to_discord(dm, wh_grp, fd.getvalue(), fd.name) if fd \
                                else push_to_discord(dm, wh_grp)
                            st.toast("🚀 Đã bắn!" if ok else "😥 Thất bại!")
            ...
            if sid == my_id:
                ...
                if tu.empty:
                    ...
                    update_cell_by_id(WS_USERS, "User_ID", my_id, "Bạn_Bè",
                                      ",".join(my_friends_list + [sid]), USER_COLS)
